  ...
  useEffect(() => {
    const redirect = async () => {
      try {
        ...
        if (!params.board || !params.class || !params.subject || !params.chapter || !params.questionId) {
        ...
        console.log('🔀 Redirecting to exercise page:', exerciseUrl);
        
        // Replace the current URL without adding to history stack
        router.replace(exerciseUrl);
      } catch (err) {
        console.error('❌ Redirect error:', err);
        setError('Failed to redirect to exercise page');
      }
    };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50">
      <div className="text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-gray-600">Loading exercise question...</p>
          <p className="text-gray-500 text-sm">Question ID: {params.questionId}</p>
        </div>
      </div>
    </div>
  );
