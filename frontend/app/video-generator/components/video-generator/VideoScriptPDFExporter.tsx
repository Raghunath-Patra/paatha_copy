...
// Updated class with proper typing and error fixes
class VideoScriptPDFExporter {
  ...
  addEnhancedSlideTitle(slide: Slide, yPos: number): number {
    ...
    return yPos + (Array.isArray(titleLines) ? titleLines.length : 1) * 8 + 10;
  }

  addEnhancedSlideContent(slide: Slide, yPos: number): number {
    if (!this.pdf) return yPos;
    ...
    if (slide.content) {
      ...
      yPos += (Array.isArray(contentLines) ? contentLines.length : 1) * 6 + 10;
    }
    ...
    if (slide.content2) {
      ...
      yPos += (Array.isArray(content2Lines) ? content2Lines.length : 1) * 6 + 10;
    }
    
    return yPos;
  }

  addEnhancedNarration(slide: Slide, yPos: number): number {
    if (!this.pdf || !slide.narration) return yPos;
    
    // Narration box
    const narrationLines = this.pdf.splitTextToSize(slide.narration, this.contentWidth - 20);
    const boxHeight = (Array.isArray(narrationLines) ? narrationLines.length : 1) * 6 + 20;
    ...
    yPos += (Array.isArray(narrationLines) ? narrationLines.length : 1) * 6 + 15;
  ...
  addEnhancedVisualDetails(slide: Slide, yPos: number): number {
    if (!this.pdf || !slide.visual?.type) return yPos;
  ...
  addSpeakerProfile(speaker: Speaker, speakerKey: string, yPos: number): number {
    if (!this.pdf) return yPos;
    ...
    // Border
    this.pdf.setLineWidth(1);
    const colorRgb = speaker.color ? this.hexToRgb(speaker.color) : [200, 200, 200];
    this.pdf.setDrawColor(colorRgb[0], colorRgb[1], colorRgb[2]);
  ...
  addContentStatistics(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  calculateTotalCharacters(): number {
    return this.slides.reduce((total, slide) => {
      let chars = 0;
  ...
  calculateContentComplexity(): string {
  ...
  calculateVocabularyDiversity(): number {
    const allWords = this.slides.reduce((words: string[], slide) => {
      let slideText = '';
  ...
  addProjectDescription(yPos: number): number {
    if (!this.pdf) return yPos;
    ...
    return yPos + (Array.isArray(descLines) ? descLines.length : 1) * 6 + 5;
  }

  addContentStructure(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addTechnicalSpecs(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addSpeakerStatistics(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addVisualFunctionDetail(funcName: string, yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addProductionStatistics(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addQualityMetrics(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addFilmingGuidelines(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addAudioGuidelines(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addPostProductionNotes(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addExportMetadata(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addVersionHistory(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addContactInformation(yPos: number): number {
    if (!this.pdf) return yPos;
  ...
  addProductionNotesForSlide(slide: Slide, yPos: number): number {
    if (!this.pdf) return yPos;
    
    // Add production-specific notes for the slide
    const notes = [];
    
    if (slide.visual?.type) {
      notes.push(`Visual rendering required: ${slide.visual.type}`);
    }
    
    if (slide.isComplex) {
      notes.push('Complex slide - allow extra production time');
    }
    
    const duration = slide.visualDuration || 4;
    if (duration > 6) {
      notes.push('Extended duration - ensure pacing is maintained');
    }
  ...
  calculateProductionComplexity(): string {
  ...
  calculateRenderingComplexity(): string {
  ...
  estimatePostProductionEffort(): string {
  ...
  assessVisualEnhancement(): number {
  ...
  assessPacingConsistency(): string {
...
declare global {
  interface Window {
    generateVideoScriptPDF: (project: Project, slides: Slide[], filename?: string) => Promise<boolean>;
  }
}

if (typeof window !== 'undefined') {
  window.generateVideoScriptPDF = async function(project: Project, slides: Slide[], filename?: string): Promise<boolean> {
    try {
      const exporter = new VideoScriptPDFExporter();
