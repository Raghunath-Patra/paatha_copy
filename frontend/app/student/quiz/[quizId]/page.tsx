...
import ImageTextExtractor from '../../../components/quiz/ImageTextExtractor';
import { 
  Clock, 
  Calendar, 
  Award, 
...
export default function TakeQuiz() {
  ...
  // Handle text extraction from ImageTextExtractor component
  const handleTextExtracted = (extractedText: string, source: 'camera' | 'upload') => {
    const currentQuestion = questions[currentQuestionIndex];
    const currentAnswer = answers[currentQuestion.id] || '';
    const separator = currentAnswer.trim() ? `\n\n--- From ${source === 'camera' ? 'Camera' : 'Image'} ---\n` : '';
    const newAnswer = currentAnswer + separator + extractedText;
  ...
  const submitQuiz = async () => {
    ...
    try {
      ...
      if (isClient && typeof window !== 'undefined' && window.sessionStorage) {
        try {
          sessionStorage.setItem(`quiz_results_${resultData.attempt.id}`, JSON.stringify(resultData));
        } catch (e) {
          console.warn('Failed to store results in sessionStorage:', e);
        }
      }
  ...
  // Don't render until client is ready for components that use browser APIs
  if (!isClient) {
    return (
            ...
            <div className="space-y-3 mb-6">
              {currentQuestion.question_type.toLowerCase() === 'mcq' && currentQuestion.options ? (
                ...
                <div className="space-y-3">
                  {/* Enhanced textarea with image upload */}
